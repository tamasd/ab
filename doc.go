...
/*
Alien Bunny is a web development kit

It fits websites where the server side rendering is minimal or does not happen at all. A good example is a single page application or a website where the main content is simple to render, and the additional widgets can load later. The kit is designed to work well with ReactJS, but it shouldn't have any issues with other similar client side frameworks (Angular, Durandal etc).
...
The kit contains APIs. These APIs are usually a middleware and a struct in the request context.
*/
package ab
